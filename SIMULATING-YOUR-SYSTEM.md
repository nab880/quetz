...
`include/quetz/quetz_ipc_types.h` is the single C/C++ ABI definition. ABI v5 uses magic `0x515A4D05`; native RAM descriptors contain guest base, size and a shared-relative backing offset. The allocating process sets offsets once. QEMU maps the same offsets even when its host mapping has a different alignment residue. The mailbox layout, reset epochs, event and result semantics remain shared across both sides.
