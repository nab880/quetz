...
void QemuFrontend::spawn(const QuetzConfig& cfg, bool detailed_tracking) {
    if (!quetz_configure_native_regions(tunnel_->getSharedData(),
            cfg.cache_native_regions.data(), cfg.cache_native_regions.size()))
        output_->fatal(CALL_INFO, -1, "Invalid shared native RAM registration.\n");
