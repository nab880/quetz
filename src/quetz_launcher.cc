...
pid_t QemuLauncher::spawn(const QuetzConfig& cfg,
    ...
    if (cfg.system_mode && cfg.vcpu_count > 1) {
        if (const char* error = multicoreLaunchError(
                cfg.vcpu_count, cfg.sst_window_cache, cfg.qemu_extra_args, cfg.platform_machine))
            output_->fatal(CALL_INFO, -1, "system multicore: %s.\n", error);
    }
    if (cfg.sst_window_cache) {
        if (const char* error = windowCacheLaunchError(cfg.qemu_extra_args, cfg.vcpu_count, cfg.platform_machine))
    ...
    if (cfg.architecture_adapter == "coldfire-v4e") {
        bool explicit_cpu = false;
        for (const auto& arg : cfg.qemu_extra_args)
            if (arg == "-cpu" || arg == "--cpu" || arg.rfind("-cpu=", 0) == 0 || arg.rfind("--cpu=", 0) == 0)
                explicit_cpu = true;
        if (!explicit_cpu) { argv_strs.push_back("-cpu"); argv_strs.push_back("cfv4e"); }
    }
