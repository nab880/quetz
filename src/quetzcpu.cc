      ...
      halted_count_(0)
{
    ...
    if (cfg_.sst_window_cache) {
        ...
        if (const char* error = windowCacheLaunchError(cfg_.qemu_extra_args, cfg_.vcpu_count, cfg_.platform_machine))
        ...
        try {
            ...
            for (const auto& region : cfg_.cache_native_regions)
                window_caches_.addRegion(region.base, region.size);
    ...
    if (cfg_.system_mode && cfg_.vcpu_count > 1) {
        if (const char* error = multicoreLaunchError(
                cfg_.vcpu_count, cfg_.sst_window_cache, cfg_.qemu_extra_args, cfg_.platform_machine))
