...
void QuetzCPU::serviceWindowCache(uint32_t vcpu)
{
    ...
    for (;;) {
        ...
        auto* shared = frontend_->tunnel()->getSharedData();
        uint8_t* native = nullptr;
        for (unsigned i = 0; i < shared->native_region_count; ++i) {
            const auto& region = shared->native_regions[i];
            if (action.address >= region.base && action.address - region.base < region.size) {
                const auto offset = action.address - region.base;
                if (action.size > region.size - offset)
                    throw std::runtime_error("cache backing access crosses native RAM boundary");
                auto* backing = quetz_native_ram(shared, i);
                if (!backing) throw std::runtime_error("invalid registered native RAM backing");
                native = backing + offset;
                break;
            }
        }
        if (native) {
