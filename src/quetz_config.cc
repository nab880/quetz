...
QuetzConfig QuetzConfig::fromParams(Params& params, SST::Output* out) {
    ...
    cfg.architecture_adapter = params.find<std::string>("architecture_adapter", "");
    cfg.platform_machine = params.find<std::string>("platform_machine", "");
    try {
        cfg.cache_native_regions = parseNativeRegions(params.find<std::string>("cache_native_regions", ""));
    } catch (const std::exception& e) { out->fatal(CALL_INFO, -1, "%s\n", e.what()); }
    ...
    if (!cfg.architecture_adapter.empty() && cfg.architecture_adapter != "coldfire-v4e")
        out->fatal(CALL_INFO, -1, "Unsupported architecture_adapter: %s\n", cfg.architecture_adapter.c_str());
    if ((cfg.sst_window_cache || (cfg.system_mode && cfg.vcpu_count > 1)) &&
        cfg.architecture_adapter != "coldfire-v4e")
        out->fatal(CALL_INFO, -1, "Cache/system multicore requires architecture_adapter=coldfire-v4e.\n");
    if (!cfg.cache_native_regions.empty() && !cfg.sst_window_cache)
        out->fatal(CALL_INFO, -1, "cache_native_regions requires sst_window_cache=1.\n");
    if (cfg.architecture_adapter == "coldfire-v4e") {
        if (!cfg.system_mode) out->fatal(CALL_INFO, -1, "ColdFire adapter requires system mode.\n");
        if (const char* error = coldFireLaunchError(cfg.qemu_extra_args, cfg.vcpu_count, cfg.platform_machine))
            out->fatal(CALL_INFO, -1, "%s\n", error);
    }
    return cfg;
}
